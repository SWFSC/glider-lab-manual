...
The [standard-glider-files](https://github.com/SWFSC/standard-glider-files) repository is the home for both all ESD glider cache files, and standard files that are put on ESD gliders before deployment. This repo and these files are also commonly used during ESD lab tests.
...
NOTE: currently, cache files are stored in both [standard-glider-files](https://github.com/SWFSC/standard-glider-files), and [GCP](https://console.cloud.google.com/storage/browser/amlr-gliders-deployments-dev/cache). In the future, cache files will only be stored in the standard-glider-files repo.
...
**plots**: Standard plots generated from either the 'processed-rt' (subfolder 'rt') or 'processed-L1' data (subfolder 'delayed'). Note that 'sci' is short for plots of science sensor values, while 'eng' is short for plots of engineering variables. See [plots](slocum-glider-data.qmd#plots) for descriptions of the various folders.
