...
These base processing details apply to both real-time and delayed-mode data. Real-time (i.e., 'rt') data means the subset of data collected while the glider is deployed, while delayed-mode (i.e., 'delayed') data is the full resolution dataset downloaded from the glider after the glider has been recovered. 

### Data Products

The esdglider package heavily leverages `dbdreader` and `pyglider` to create several different base processing data products, which are described in this section. For descriptions of the various standard plots, see the [Plots](#plots) section. See the individual base processing sections for specific processing steps, choices, etc.
...
3. Create a [deployment-specific processing script](https://github.com/SWFSC/glider-lab/tree/main/deployment-scripts), and run it in a GCP Workbench Instance. This script creates NetCDF files, acoustic data procucts (if necessary), imagery data products (if necessary), and standard plots.
