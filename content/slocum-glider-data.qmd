...
The `esdglider` toolbox heavily leverages `dbdreader` and `pyglider` to create several different 'L1' base processing data products, which are described in this section. For descriptions of the various standard plots, see the [Plots](#plots) section. See the individual base processing sections for specific processing steps, choices, etc.
...
- **acoustic metadata files**: Files needed for processing acoustic data with Echoview. See the [active acoustics](active-acoustics.qmd) page for more details. These files, which live in the glider active acoustics bucket (e.g. [here](https://console.cloud.google.com/storage/browser/amlr-gliders-acoustics-dev/SANDIEGO/2022/amlr08-20220513/metadata/echoview)), are only created if the glider is carrying an active acoustic instrument, such as a Nortek or AZFP
