...
Both echosounders are bottom looking so the processing is very similar. Currently, data from the glider deployment will need to be processed first. See Glider Data processing (index page) on how ESD processes glider data. The data needed from the glider are date, time, GPS, depth, pitch, and roll. 
